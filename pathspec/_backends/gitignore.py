"""
This module provides backend utility functions used by :class:`~pathspec.gitignore.GitIgnoreSpec`.
...
def make_backend(
	...
	*patterns* (:class:`.Iterable` of :class:`.Pattern`) contains the compiled
	...
	if name == 'hyperscan':
		return HyperscanGiBackend(cast(Iterable[RegexPattern], patterns))
	elif name == 'simple':
		return SimpleGiBackend(patterns)
	else:
